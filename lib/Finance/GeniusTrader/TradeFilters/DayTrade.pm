package Finance::GeniusTrader::TradeFilters::DayTrade;
...
sub accept_trade {
    ...
    unless ($prev_day_ptr->{$i}) {
        ...
        while (--$x && !$prev_day_ptr->{$i}) {
            ...
            if (Finance::GeniusTrader::DateTime::convert_date($pi->[$DATE], $calc->prices->timeframe, $DAY) ne
                    Finance::GeniusTrader::DateTime::convert_date($p->[$DATE], $calc->prices->timeframe, $DAY)) {
                $prev_day_ptr->{$i} = $x;
            }
            else {
